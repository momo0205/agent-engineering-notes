...
LLM 是一个外部推理能力，API Key 只是访问凭据；Context 是一次决策能看到且被允许相信的全部信息；Harness 是围绕模型的确定性控制系统。Context 和 Harness 都不是单一文件，而是跨越检索、协议、状态、校验、测试和运行时配置的一组边界。
...
所以，答案是：ReAct 与 Loop 已写，但只是一条受限、可验证的垂直切片。它证明架构方向成立，还没有证明通用自主执行能力成立。
