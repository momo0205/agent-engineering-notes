...
但这些还不足以称为 production-ready。当前已经有 PostgreSQL Checkpoint、重启恢复和生产 Trace；Token 预算尚未按供应商实际用量扣减；工具异常没有统一分类、退避和重规划；对外运行、查询、取消和恢复 API 也尚未开放。
...
另一个坑是把内存里的 `AgentStepTrace` 当成可观测性完成。它对测试很有价值，但服务重启后无法恢复，也不能按运行查询模型用量和工具调用。这个问题后来通过持久化 Trace、关联 ID、敏感数据策略和查询接口得到修正，但仍不能替代真实预算和通用异常恢复。
...
M3 仍在进行。后续至少要完成：按真实 usage 扣减 Token/费用；工具超时、失败和部分结果的分类恢复；认证的运行、状态、结果、取消与恢复 API。
